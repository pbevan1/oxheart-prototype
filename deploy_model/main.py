...
def get_newest_model_url(project_id, location, model_display_name):
    ...
    models = aiplatform.Model.list(filter=filter_expression, order_by="create_time desc")
    ...
    if models:
        ...
        model_path = parsed_uri.path.lstrip('/')+'/model.joblib'.lstrip(parsed_uri.netloc)
...
model_url, bucket_name = get_newest_model_url(PROJECT_ID, args.LOCATION, args.PIPELINE_NAME)
...
def predict():
    if request.method == 'POST':
        ...
        features_str = request.args.get('features', '')  # Get data from query parameters for GET requests
